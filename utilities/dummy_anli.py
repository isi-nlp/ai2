...
# Load in Train
train = []
with open('../task_data/alphanli-train-dev/increment_training/2/train.jsonl', 'r') as train_file:
...
with open('../task_data/alphanli-train-dev/dummy/train-labels.lst', 'w') as output:
    for a_label in label[:-dev_count]:
        output.write(f'{a_label}\n')
with open('../task_data/alphanli-train-dev/dummy/train.jsonl', 'w') as output:
    for a_train in train[:-dev_count]:
        output.write(f'{json.dumps(a_train)}\n')
with open('../task_data/alphanli-train-dev/dummy/dev-labels.lst', 'w') as output:
    for a_label in label[-dev_count:]:
        output.write(f'{a_label}\n')
with open('../task_data/alphanli-train-dev/dummy/dev.jsonl', 'w') as output:
    for a_train in train[-dev_count:]:
        output.write(f'{json.dumps(a_train)}\n')
