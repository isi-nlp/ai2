...
class Classifier(pl.LightningModule):
    ...
    def forward(self, batch):
        print('forward_labels', batch['labels'])
        print('forward_num_choice', batch['num_choice'])
    ...
    def collate(self, examples):
        ...
        data = {"input_ids": results["input_ids"],
    ...
    def training_step(self, batch, batch_idx):
        ...
        print('training_step', logits)
        return {"out": logits,
                "labels": batch["labels"],
                "num_choice": batch["num_choice"]}

    def training_step_end(self, batch_parts_outputs):
        print('training_step_end', batch_parts_outputs)
    ...
    def validation_step(self, batch, batch_idx):
        logits = self.forward(batch)
        print('validation_step', logits)
        return {"out": logits,
                "labels": batch["labels"],
                "num_choice": batch["num_choice"]}

    def validation_step_end(self, batch_parts_outputs):
        print('validation_step_end', batch_parts_outputs)
        logits = batch_parts_outputs["out"]
        num_choice = batch_parts_outputs["num_choice"].flatten()[0].item()
        logits = logits.reshape(-1, num_choice)
        loss = self.loss(logits, batch_parts_outputs["labels"][0])
